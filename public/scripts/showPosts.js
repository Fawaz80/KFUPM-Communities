getPosts();
var user = (document.getElementsByClassName("btn btn-secondary dropdown-toggle")[0].innerText).trim();
...
document.getElementsByClassName("create")[0].addEventListener("click", (e) => {
    if (isUser) {
        ...
        .then(response => response.json())
        .then(communities => {
            var selecter = document.getElementById("toInput")
            for (let i = 0; i < communities.length; i++) {
                selecter.innerHTML += `<option>${communities[i].comName}</option>`
            }
            if (communities.length == 0)
                selecter.innerHTML += `<option>No Community There, How About Creating One?</option>`;
        })
        .catch(error => console.error('Error:', error));
        return;
    }
    ...
    for (let i = 0; i < posts.length; i++) {
        ...
        `<div class="postDiv">
            ...
            <div class="bottomPost">
                <div class="postInfo">
                    <p>posted <span class="dateOfPosting">${post.postDate} </span> to<strong> @<a href="community.html" class="communityTarget">${post.postCom}</a></strong></p>
                </div>
